...
void insereLista(){
    ...
    auxiliar -> dado = valor;
    //manipulação dos ponteiros para completar a inserção
...
void removeLista(){
    no *indice = cabecalho->proximo, *anterior=cabecalho;
    int valor;
    ...
    if(cabecalho->proximo == NULL){
        ...
        for(int i=0; i<cabecalho->dado; i++){
            if(indice->dado == valor){ //testa se algum dos valores da lista é igual ao valor a ser removido.
                //manipulação do ponteiro para a remoção e liberação da memória que estava alocada:
